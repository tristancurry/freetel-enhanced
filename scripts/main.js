...
let pos_injection = {x:0, y: 0, z:0}; //position in plate region where particles enter
...
let dir_injection = {x: Math.cos(toRadians(alt_injection))*Math.cos(toRadians(az_injection)), y: Math.sin(toRadians(alt_injection)), z: Math.cos(toRadians(alt_injection))*Math.sin(toRadians(az_injection))};
...
Particle.prototype.render = function (target_width = l_screen, target_height = d_plates) {
    ...
    ctx_exp.beginPath();
    ctx_exp.fillStyle = 'rgb(255,255,0)';
